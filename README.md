...
Edge/Chrome 등 브라우저가 "이 파일 형식은 일반적으로 다운로드되지 않습니다" 같은 경고를 띄우며 파일을 자동으로 지우는 경우가 있어요. 배포용 인증서(코드 서명)가 없는 개인 제작 프로그램이라 생기는 안내일 뿐, 악성 파일이라서 뜨는 경고가 아니에요.
...
이렇게 하면 파일이 지워지지 않고 정상적으로 다운로드가 완료돼요.
